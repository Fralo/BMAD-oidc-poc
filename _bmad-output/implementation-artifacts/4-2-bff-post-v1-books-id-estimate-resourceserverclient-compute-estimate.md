...
Generated by `bmad-code-review` on 2026-05-17 against commit `e3236bd` (baseline `e167c3c`). Three layers (Blind Hunter, Edge Case Hunter, Acceptance Auditor) returned 27 findings; triaged to 7 patches, 8 defers, 12 dismissals. Acceptance Auditor verified all 15 ACs faithfully — no AC violations.

**Patches (applied 2026-05-18):**

- [x] [Review][Patch] **Forwarded RS body may be `None` → JSON `null` to SPA** [services/bff/src/bff/services/resource_server_client.py:317-332] — fix centralized in `_do_rs_call`: empty response body now raises `RsUnavailable("rs_empty_body", http_status=…)` for every status code EXCEPT 401 (where OAuth 2.0 / RFC 6750 challenges legitimately omit a body and the refresh-and-replay logic consumes the bare `(401, None)` tuple). The malformed body never reaches the BFF handler, so the SPA never sees JSON `null`. Two new tests pin the new branch: `test_rs_200_with_empty_body_emits_rs_unavailable`, `test_rs_412_with_empty_body_emits_rs_unavailable`. (Med, blind+edge)
- [x] [Review][Patch] **Cookie-clearing assertion: `max-age=0` checked per cookie** [services/bff/tests/api/test_books.py:`test_estimate_refresh_failure_clears_cookies_and_returns_401`] — replaced joined-string check with a per-cookie loop; a regression clearing only one of the two cookies now fails the assertion. (Med, blind)
- [x] [Review][Patch] **NFR6 tests now scan request headers for sub leakage** [services/bff/tests/api/test_books.py:`test_estimate_nfr6_captured_rs_request_shape`; services/bff/tests/services/test_resource_server_client.py:`test_compute_estimate_no_sub_in_url_query_or_body`] — replaced the dead `"sub" not in body` line (subsumed by the prior `body == {"pages": N}` equality) with `assert not any(sub in v for v in request.headers.values())`. NFR6's "sub never leaks anywhere" now pins URL path + query + body + headers. (Med, blind)
- [x] [Review][Patch] **`assert_all_called=False` removed from happy-path / replay tests** [services/bff/tests/api/test_books.py; services/bff/tests/services/test_resource_server_client.py] — 13 happy-path tests in `test_books.py` and 11 in `test_resource_server_client.py` flipped to the default (`assert_all_called=True`). The 7+2 negative-path tests that intentionally never hit the registered mock (and assert `call_count == 0`) keep `=False`. URL typos in mock setup now fail the test. (Med, blind)
- [x] [Review][Patch] **Handler docstring clarified — `pages` comes from `book.pages`, request body is intentionally ignored** [services/bff/src/bff/api/books.py:`estimate_for_book` docstring step 3]. (Low, blind)
- [x] [Review][Patch] **Expired-session route test asserts `rs_route.call_count == 0`** [services/bff/tests/api/test_books.py:`test_estimate_expired_session_returns_401_and_deletes_row`]. Aligns with sibling 401 tests and pins AC11 "no RS call in 401 paths". (Low, auditor)
- [x] [Review][Patch] **`refresh_succeeds_retry_still_401` verifies access_token + refresh_token rotation** [services/bff/tests/services/test_resource_server_client.py:`test_compute_estimate_refresh_succeeds_retry_still_401`]. Pins AC9 step 3a on the retry-401 branch too. (Low, auditor)

**Deferred (pre-existing patterns or out-of-scope polish — also appended to deferred-work.md):**

- [x] [Review][Defer] **Inlined `_session_terminated_response` / `_resource_server_unavailable_response` helpers duplicate `api/reading_speed.py`** [services/bff/src/bff/api/books.py:128-166] — spec AC's design note explicitly defers the extract-to-shared-module decision to "after a fourth consumer arrives" (3.5/4.2 = consumers two/three). Also `_clear_session_cookies` is imported lazily from `bff.api.auth` (private API) to dodge the circular import — the right fix is a neutral `api/_cookies.py` module, deferred to the same trigger. (Med, blind+edge, pre-existing pattern from 3.5)
- [x] [Review][Defer] **503 envelope drops `RsUnavailable.cause` — no operator correlation** [services/bff/src/bff/api/books.py:`_resource_server_unavailable_response`] — `RsUnavailable` carries `cause` and `http_status`; the BFF envelope hardcodes `detail: None`. WARN log inside `ResourceServerClient` exists but no request-id correlation reaches the SPA. Sibling helper in `reading_speed.py` has the same gap — observability polish pass. (Low, blind)
- [x] [Review][Defer] **`_resolve_session_row` lazy-delete + raise: DB error during cleanup replaces intended 401 with 500** [services/bff/src/bff/api/books.py:118-120] — pre-existing pattern (identical to `_resolve_session_sub` and `api/reading_speed.py:_require_session`); AC2 explicitly mandates verbatim copy. Real fix: try/except around `delete_expired_session(...)` so the 401 still emits if the cleanup blips. Belongs to a shared session-resolution helper extraction. (Low, blind+edge, pre-existing)
- [x] [Review][Defer] **`book_id` path int has no upper bound — Postgres `INTEGER` (4-byte signed) overflow at `> 2_147_483_647` raises `DataError` → 500** [services/bff/src/bff/api/books.py:248,266] — pre-existing pattern on every book handler (read/update/delete already share the gap). SQLite-only deployments hide it. Real fix: `book_id: Annotated[int, Path(ge=1, le=2_147_483_647)]` repo-wide. (Low, edge, pre-existing)
- [x] [Review][Defer] **`_books_service.get_for_user` DB exception not caught (connection lost, OperationalError)** [services/bff/src/bff/api/books.py:266] — pre-existing across all books read paths; new estimate route inherits the gap. Real fix: shared error-mapping decorator that translates SQLAlchemy errors to a 503 envelope. (Low, edge, pre-existing)
- [x] [Review][Defer] **`_persist_refreshed_tokens` DB commit failure after Keycloak rotation loses fresh tokens → phantom logout next request** [services/bff/src/bff/services/resource_server_client.py:263] — pre-existing from Story 3.5; new POST route makes it the third reachability vector. The refresh_token has already been consumed at Keycloak when the DB commit fails — next request retries the now-revoked token and forces logout. Real fix: try/except around `_persist_refreshed_tokens` that either retries the commit or raises `RsSessionTerminated(clear_cookies=True)` with a "re-auth required" envelope. (Med, edge, pre-existing)
- [x] [Review][Defer] **Concurrent refresh race on same session — second consumer of stale `refresh_token` triggers `RsSessionTerminated` despite first request succeeding** [services/bff/src/bff/services/resource_server_client.py:240-263] — pre-existing from Story 3.5; SPA mounts `EstimateCell` + reading-speed widget simultaneously after idle could fire the race. No per-session lock or row-level coordination. Real fix: `asyncio.Lock` keyed on `session_row.id` or `SELECT ... FOR UPDATE` on the session row at refresh time. (Med, edge, pre-existing)
- [x] [Review][Defer] **POST `/v1/estimate` replay on RS-401 assumes RS endpoint is idempotent — assumption not pinned in `compute_estimate` docstring or in `_call_with_refresh` signature** [services/bff/src/bff/services/resource_server_client.py:265-267] — today's RS `/v1/estimate` is read-only computation, but `_call_with_refresh` will happily replay any POST. If the RS ever extends `/v1/estimate` to write a row (history, telemetry), a 401-then-replay would double-write. Real fix: docstring note + optional `replay_safe: bool = True` flag on `_call_with_refresh`. (Low, edge)

**Dismissed (12 — noise / false positive / handled elsewhere):**

- Handler forwards every non-401 RS status verbatim with no allow-list — **spec AC4–AC7 + Dev Notes "verbatim proxy"** explicitly mandate the pass-through. (blind)
- `rs_settings` monkeypatch + module-level `resource_server_client` singleton is "brittle to a future refactor" — speculative; tests pass under current import pattern. (blind)
- `test_estimate_non_integer_book_id_returns_422` assumes a global handler rewrites FastAPI's default 422 envelope — verified: `bff/core/errors.py:33` defines `INVALID_INPUT` and the test fleet passes green. (blind)
- `assert b"600.0" not in raw` is single-value smoke — name is accurate enough; broader bytes-regex adds noise. (blind)
- `inspect.signature` test for keyword-only `pages` vs `pytest.raises(TypeError)` — equivalent coverage; structural assertion is fine. (blind)
- Route-level 5xx parametrization narrower than service-level (route: 500/503; service: 500/502/503/504) — AC12 explicitly sanctions parametrization-across-cases; route-level relies on service-level coverage. (blind, auditor)
- No outgoing-body Pydantic validation on `{"pages": pages}` — over-engineering; RS validates, and book.pages is typed `int` from ORM. (blind)
- `compute_estimate` accepts arbitrary `pages: int` (no `ge=1` bound) — RS rejects with 422, defensive locality not worth the noise. (blind)
- `_resource_server_unavailable_response()` takes no `cfg` while sibling takes `cfg` — minor asymmetry, `cfg` is only needed for cookie-clearing branch. (blind)
- No test for RS 200 with malformed (non-JSON / non-dict) body — already pinned at `_do_rs_call` level by Story 3.5 CR10 (`rs_malformed_body` branch); covered transitively. (blind)
- `book.pages = None` forwarded to RS — `BookCreate` enforces `ge=1` at write time; defending against pre-Pydantic seed rows is out-of-scope. (edge)
- `_resolve_session_row` is a 12-line duplicate of `_resolve_session_sub` — spec AC2 design note explicitly chose this over premature consolidation. (auditor implicit)
