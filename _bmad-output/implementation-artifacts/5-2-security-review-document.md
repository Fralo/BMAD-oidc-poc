...
Code review of Story 5.2 — three adversarial layers (Blind Hunter + Edge Case Hunter + Acceptance Auditor) ran in parallel against the diff at `4bafc5f..fb751ec`. AC10 / DoD #7 returned FAIL on the load-bearing "defer IDs resolve to `deferred-work.md`" invariant — 7 of 19 Known-Gaps citations name D-IDs that do not exist in the registry (max actual: D135). Independently corroborated by Acceptance Auditor and Edge Case Hunter. Architecture line citations are systematically off by ~40 lines.

**Decision needed (1):**

- [x] [Review][Decision] **AC10/DoD#7 FAIL — 7 fabricated D-IDs in Known Gaps** — D369, D371, D372, D440, D441, D442, D470 don't exist in `_bmad-output/implementation-artifacts/deferred-work.md` (highest actual: D135). AC10's "every entry cites its defer ID so a maintainer can navigate to the full description" affordance breaks for ~37% of Known-Gaps items. Three valid fixes: (a) backfill `deferred-work.md` with proper D369/D371/D372/D440/D441/D442/D470 entries; (b) rewrite those 7 Known Gaps items without D-citations (cite severity + affected file only); (c) hybrid — map to existing IDs where they fit, create new ones for the rest. Spec line 44 also propagates the same phantom IDs, so the fabrication originated in the spec.

**Patches (10):**

- [x] [Review][Patch] **Architecture line citations wrong by ~40 lines** [docs/security-review.md multiple sections] — `architecture.md:76` cited for "Token isolation" (actual: line 36); `:80` for "Public keys are not hardcoded" (phrase doesn't exist; JWKS bullet is line 40); `:82` for "Scope-enforced computation" (actual: line 42); `:83` for "BFF does not bypass the resource server" (actual: line 43). At least four call sites.
- [x] [Review][Patch] **`resource_server_client.py:178–224` cited for refresh-and-replay** [docs/security-review.md §1] — lines 178–224 are public wrappers (`get_reading_speed`, `put_reading_speed`, `compute_estimate`). Actual `_call_with_refresh` starts at line 230, `_refresh_access_token` at line 359.
- [x] [Review][Patch] **`auth.py:119–127` and `:119–129` labeled "clear at logout" — actually state-cookie clear on `auth_state_invalid` error path** [docs/security-review.md §1 reference list + §2 prose] — Logout's cookie-clear is at lines 347+ (the `set_cookie` calls in the logout response). Three different ranges given for "logout clear" (119–127, 119–129, 285–295) and none of them is the logout path.
- [x] [Review][Patch] **Set-cookie line range inconsistent: `297–305` (§1 code-block) vs `298–305` (§2 prose)** [docs/security-review.md §1 + §2] — actual `redirect.set_cookie(` for the session cookie is at line 298. Pick one form throughout.
- [x] [Review][Patch] **AR3 cross-reference "§9 threat 6 below" — there is no §9** [docs/security-review.md AR3] — intended target is T6 in "Threat Model Summary". A reader following the reference lands on §6 (CSP) instead.
- [x] [Review][Patch] **"urlsplit silently strips userinfo" — technically wrong** [docs/security-review.md §3] — `urlsplit` preserves userinfo in `netloc`; only `.hostname` strips it. Code at `csrf.py:169–170` actually checks `parts.username or parts.password` to reject. Re-word to be precise.
- [x] [Review][Patch] **"TTL 24h" overstated as JWKS cache invariant** [docs/security-review.md §4] — kid-miss re-fetch is NOT a TTL surrogate; a rotated key with no intervening traffic is held indefinitely. Soften the equivalence claim.
- [x] [Review][Patch] **§4 implies `require=["iss","aud","exp","sub"]` enforces non-empty `sub`, but PyJWT only checks presence** [docs/security-review.md §4] — contradicts Known Gap #5 which explicitly says empty `sub` → 412 (D67). Clarify §4 wording to "presence-only check."
- [x] [Review][Patch] **D369 mentioned in §6 prose ("tracked as D369 under Known Gaps") but absent from the Known Gaps list** [docs/security-review.md §6 + Known Gaps] — broken self-reference. Resolves with the Decision item above (D369 is one of the 7 fabricated IDs).
- [x] [Review][Patch] **D80 "fix: per-session `asyncio.Lock`" insufficient under multi-replica BFF** [docs/security-review.md Known Gap #6] — `asyncio.Lock` is per-process; horizontal scaling needs DB-level `SELECT … FOR UPDATE`. Acknowledge the multi-replica limit or prefer the DB alternative.

**Deferred (pre-existing, not introduced by 5.2):**

- [x] [Review][Defer] **`D63` duplicated in `deferred-work.md`** [_bmad-output/implementation-artifacts/deferred-work.md lines ~502 + ~584] — deferred, pre-existing — same ID used for BookList test stub AND WWW-Authenticate header gap. Registry corruption that pre-dates Story 5.2; resolving it belongs in a deferred-work-cleanup pass.
- [x] [Review][Defer] **D84 line-range citation drift in `deferred-work.md`** [_bmad-output/implementation-artifacts/deferred-work.md D84 entry] — deferred, pre-existing — D84's own source-line citations (`196-201, 219-222, 335-339`) drifted out of sync with `resource_server_client.py`; actual `session_row.id[:8]` log sites are at lines 255 / 276 / 433. Pre-existing in deferred-work.md, not Story 5.2's diff.

**Dismissed as noise (~12):** Python 3.14 `except A, B:` syntax dismissal defensible (3.14 runs it correctly); sprint-status.yaml narrative discrepancy (+4 / +8/-4 vs actual git stat) is non-load-bearing YAML comment metadata; AR2 phrasing "no user identity" adequate; severity "nit" vs "low" semantically clear; Story 1.13 J5 reference plausible (verified in deferred-work + git history); AR1 JWE-cookie alternative is meant as architectural option list, not a self-contradiction with §2; dependency hygiene table mixed pinning forms clear enough for the dep-hygiene point; commit-SHA-in-header is the worktree parent (acknowledged by dev with explanatory note; defensible workflow artifact); AR1 plaintext-tokens primary-vs-secondary framing; T4 "intentionally accepted" framing clear; etc.
...
| 2026-05-18 | 1.1 | Code review pass complete; status `review` → `done`. Three-layer adversarial review (Blind Hunter + Edge Case Hunter + Acceptance Auditor) ran in parallel. Acceptance Auditor returned PASS on AC1–AC9 + AC11 + 14/14 failure-prevention items + 12/13 DoD items; **AC10 / DoD #7 returned FAIL** on the load-bearing "defer IDs resolve to `deferred-work.md`" invariant — 7 of 19 Known-Gaps citations named D-IDs that did not exist in the registry (D369, D371, D372, D440, D441, D442, D470; max actual at review time was D135). User selected the "backfill" resolution path. 11 patches applied (1 from decision-needed resolution, 10 from direct findings): D1 backfilled `deferred-work.md` with 7 new entries under a "Deferred from: code review of 5-2-..." section preserving the same D-IDs the document cites; P1 corrected 4 architecture line citations (`:76 → :36` Token isolation, `:80 → :40` JWKS / no hardcoded keys, `:82 → :42` Scope-enforced computation, `:83 → :43` BFF does not bypass) — systematic ~40-line offset; P2 corrected `resource_server_client.py:178–224` (public wrappers) → `:230–279` (`_call_with_refresh`) + `:359–410` (`_refresh_access_token`); P3 relabeled `auth.py:119–127` from "logout cookie clear" to its actual purpose (state-cookie clear on `auth_state_invalid` error response) and added the real logout-clear ranges (`:347–355` session + `:356–364` CSRF); P4 standardized session-cookie set range to `:298–305` (was inconsistent `:297–305` vs `:298–305`); P5 retargeted AR3 cross-reference "§9 threat 6 below" → "T6 in Threat Model Summary below"; P6 corrected the §3 `urlsplit` claim — `urlsplit` preserves userinfo in `netloc`; only `.hostname` strips it; the code at `csrf.py:169–170` checks `parts.username or parts.password` to reject; P7 softened the §4 JWKS-TTL equivalence — `kid`-miss re-fetch satisfies the key-rotation half of architecture line 415 but NOT the 24h time-based TTL half (a cached key with no intervening traffic is held indefinitely, not for 24h); P8 added a presence-only note to §4's required-claims gate (PyJWT's `require` enforces presence, not non-empty values — explicitly cross-references Known Gap #5 / D67 for the empty-`sub` consequence); P9 in-section reference to D369 was re-routed to `deferred-work.md` directly (D369 is now in the registry, so the §6 prose reference resolves verbatim — Known Gaps list count remains 19, exceeding the AC10 floor of 18); P10 updated Known Gap #6 (D80 refresh-token rotation race) fix narrative — prefer DB-level `SELECT … FOR UPDATE` for multi-replica correctness; acknowledged `asyncio.Lock` is single-replica only. 2 defers logged in the story (W1 `D63` duplicated in `deferred-work.md` + W2 D84 source-line citation drift) — both pre-existing in the registry, not introduced by 5.2. ~12 findings dismissed as noise (Python 3.14 `except A, B:` syntax dismissal defensible; sprint-status narrative `+4 / +8/-4` discrepancy is non-load-bearing YAML metadata; AR2 phrasing adequate; severity "nit" label intentionally consistent across same-class items; Story 1.13 J5 ref plausible; AR1 JWE-cookie alternative meant as architectural option list; dep-hygiene table mixed pinning forms clear enough; commit-SHA-in-header is worktree-parent with explanatory note; etc.). Static gates: doc structure unchanged (still 9 H2 sections matching AC1; all 31 relative links resolve; CSP byte-for-byte still verbatim from `security_headers.py:17–21`). No production source / tests / infra touched. Net diff (review pass only): `docs/security-review.md` (citation tightening across 12 sites), `_bmad-output/implementation-artifacts/deferred-work.md` (+7 new D-entries under 5.2 review section), this story file (Review Findings + Status flip + this Change Log entry). Epic 5 progress after 5.2 done: 2/4 stories done [5.1, 5.2]; 5.3 (README polish + AI integration log) and 5.4 (smoke run) still backlog. | claude-opus-4-7 |
