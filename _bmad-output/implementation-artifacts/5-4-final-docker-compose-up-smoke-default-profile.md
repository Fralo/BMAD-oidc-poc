...
- [x] **Task 4 — Run the smoke [Mode B chosen]**
  ...
  - [x] 4.2 Bring-up complete: `docker compose down -v` (idempotent — no prior project state), then `docker compose --profile default up --build -d` (NOT bare `docker compose up` — see D136). Three images built (BFF stage-0 SPA build hit warm cache), all three services reached `(healthy)` per `docker compose ps` after ~32–42 s post-start.
  ...
  - [x] 4.5 Run Record filled: date 2026-05-18, SHA `fb751ec`, operator claude-opus-4-7, host darwin 25.4.0 / Docker 29.4.3 / Compose v5.1.3, four anomalies (PENDING-operator + D136 + D137 + D138 + D139), verdict = `PASS WITH ANOMALIES`.
...
- [x] **Task 5 — Regression sweep (DoD)**
  ...
  - [x] 5.2 `git diff --stat`: `README.md` +2 (one blank + one ref line); `deferred-work.md` +13 (D136–D139); `sprint-status.yaml` +4/-2 (status flip + last_updated note from create-story turn + dev-pass turn). No drift outside the permitted set.
...
- [x] **Task 6 — Capture deferred items (DoD)**
  - [x] 6.1 Four new defers logged in `_bmad-output/implementation-artifacts/deferred-work.md` under "Deferred from: dev of 5-4-..." section: D136 (bare-`docker compose up` doesn't start default profile; medium), D137 (per-service `.env` files required; medium), D138 (default profile leaves `AUTH_TYPE=none`; medium-to-high — security degradation in the canonical bring-up), D139 (`/api/me` 401 `message` wording nit). All four cite "Belongs to" + severity + real-fix proposals per the project's defer-format convention.
  ...
  - [x] 6.3 N/A — verdict is PASS WITH ANOMALIES, not PASS, because of the PENDING-operator browser walk-through. The four defers don't escalate the verdict to FAIL because the Mode-B probes all passed; they document operator-pain workarounds (D136 / D137 / D138) and a doc-wording nit (D139) that would block a clean Mode-A first-run but don't break the runtime once the workarounds are applied.
...
$ # D137 workaround: per-service .env files required.
$ cp .env.example .env
$ cp services/bff/.env.example services/bff/.env
$ cp services/resource-server/.env.example services/resource-server/.env
$ # D138 workaround: enable oidc_bearer on RS for default profile (smoke would otherwise auth-degrade).
$ sed -i.bak 's/^#AUTH_TYPE=oidc_bearer/AUTH_TYPE=oidc_bearer/' services/resource-server/.env

$ docker compose --profile default up --build -d        # D136 workaround: explicit --profile flag
...
- **Four new defers logged (D136 / D137 / D138 / D139).** The smoke surfaced three doc-vs-code drifts that block a first-time operator bring-up and one minor wording nit:
  - **D136** — bare `docker compose up` no longer starts the default profile because all services declare `profiles: [default, dev, e2e]`; canonical form is `docker compose --profile default up --build`. README + architecture + epic AC all carry the stale form; smoke doc spells out the workaround in its "Known setup workarounds" callout.
  - **D137** — per-service `.env` files (`services/bff/.env`, `services/resource-server/.env`) are required in addition to the repo-root `.env`. Compose's `env_file: ../services/bff/.env` directive errors out with `env file ../services/bff/.env not found` if the operator follows only the repo-root `.env.example` instructions.
  ...
  - **D139** — `/api/me` 401 envelope `message` wording: `Session expired or not present`. Documentation-consistency nit; smoke doc placeholder draft had a different wording, reconciled to actual at write time.
...
- **Per-service `.env` files + `services/resource-server/.env.bak` are all gitignored.** Verified via `git status` — they do not appear in `??` entries. The `.env.bak` was a side-effect of the `sed -i.bak` D138 workaround; it stays in the working tree but does not affect git state.
...
- `docs/smoke-run.md` — operator-driven smoke checklist + Run Record + Mode-B HTTP-probe transcript + Operator follow-up checklist + Known-setup-workarounds callout (D136 / D137 / D138 mitigation).
...
- `_bmad-output/implementation-artifacts/deferred-work.md` — appended "Deferred from: dev of 5-4-..." section with D136 / D137 / D138 / D139 (+13 lines net; D135 remains the prior-high; new high is D139).
...
- `.env`, `services/bff/.env`, `services/resource-server/.env`, `services/resource-server/.env.bak` — all gitignored per repo-root `.gitignore`. The `.bak` is a side-effect of the `sed -i.bak` D138 workaround.
