...
## Deferred from: code review of 5-3-readme-polish-ai-integration-log (2026-05-18)

- **D136 — OIDC env var enumeration in README Setup step 3 may be incomplete or over-complete relative to OIDC discovery** — `README.md:23`. Setup step 3 lists `OIDC_ISSUER_URL` / `OIDC_AUTHORIZE_URL_BROWSER` / `OIDC_JWKS_URL` as the OIDC-related variables an operator may need to override. If `OIDC_ISSUER_URL` powers discovery (the typical OIDC client pattern), then `OIDC_JWKS_URL` and `OIDC_AUTHORIZE_URL_BROWSER` should be derivable from `/.well-known/openid-configuration` — naming all three implies discovery is not used. Conversely, if discovery is not used, the list is missing the token endpoint (`OIDC_TOKEN_URL`) and end-session endpoint (`OIDC_END_SESSION_URL`) that any OIDC client also needs. **Real fix:** read `services/bff/src/bff/auth/keycloak_cookie_session.py` (and the BFF OIDC plugin config) to determine whether discovery is used; if yes, drop the JWKS/authorize entries from the README's "minimum overrides" list; if no, add token + end-session. Cross-check against `.env.example`. **Belongs to:** README OIDC env var accuracy pass. **Severity:** low (`.env.example` is the source of truth; first-time readers reach the same `.env.example` step regardless).
