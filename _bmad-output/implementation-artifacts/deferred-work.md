...
## Deferred from: dev of 5-4-final-docker-compose-up-smoke-default-profile

Mode-B smoke against the default profile (2026-05-18, baseline `fb751ec`) surfaced four real findings — three are doc-vs-code drifts that block a first-time-operator bring-up; one is a documentation-consistency note. Captured here per Story 5.4 Task 6.1.

- **D136 — Bare `docker compose up` does not start the default profile** — `docker-compose.yml` + `compose/infra.yml` + `compose/app.yml` declare `profiles: [default, dev, e2e]` on every service. Compose v2/v5 with `include:` excludes profiled services unless `--profile <name>` is explicitly set, so running `docker compose up` literally (with no flag) exits with `no service selected` and starts nothing. The README (line 4 + line 30), the architecture (`_bmad-output/planning-artifacts/architecture.md:1294`), and the epic AC for Story 5.4 step 4 all say `docker compose up` without the flag — three places drift in lockstep. **Real fix:** (a) drop the `default` profile entirely so the three services run without flags (smallest diff — they would still be selected by `--profile dev` / `--profile e2e` because compose treats profile-less services as always-on), OR (b) document the canonical form everywhere as `docker compose --profile default up --build` / `COMPOSE_PROFILES=default docker compose up`, OR (c) set `COMPOSE_PROFILES=default` in `.env.example` so Compose picks it up automatically. **Belongs to:** Story 5.3 (README rewrite) — fold the fix into the new "Prod-shaped workflow" section + a parallel architecture-doc correction. **Severity:** medium (operator-facing — first invocation appears to silently no-op; documented setup steps don't work as written).

- **D137 — Per-service `.env` files are required, not just the repo-root `.env`** — `compose/app.yml:34-35` declares `env_file: ../services/bff/.env` on the BFF and `compose/app.yml:86-87` declares `env_file: ../services/resource-server/.env` on the RS. The repo-root `.env.example` header (lines 2-3) describes the file as the single canonical entry point — "Copy this file to `.env` at the repo root before running `docker compose up`. Every variable here is consumed by one or more compose services". The actual implementation diverged: each service reads its own per-service `.env` (with templates at `services/bff/.env.example` and `services/resource-server/.env.example`), and the repo-root `.env` is consumed only for variable interpolation in the realm JSON + a few compose-level `${VAR:?}` checks. First-time operator who copies only the repo-root `.env.example` (per README step 3) gets `env file ../services/bff/.env not found` before any container starts. **Real fix:** (a) consolidate to a single root `.env` and update `compose/app.yml` to use the root file for both services (smallest correct fix; per-service `.env` files become aliases or are dropped), OR (b) document the three-copy setup in both `.env.example` header and README step 3, OR (c) ship a `Justfile` recipe `just env-init` that copies all three from their templates. **Belongs to:** Story 5.3 (README rewrite) or a fast-follow setup-hygiene story. **Severity:** medium (blocks first-time bring-up entirely).

- **D138 — Default profile leaves `AUTH_TYPE=none` on the Resource Server** — `services/resource-server/.env.example:57` has `#AUTH_TYPE=oidc_bearer` commented out; the .env-example comment (lines 54-56) acknowledges this: "Compose profiles default + dev + e2e SHOULD set AUTH_TYPE=oidc_bearer (the actual wiring flips in Story 3.3 / 3.6 once a scope-gated handler exists to consume it)". Story 3.3 / 3.6 landed and the scope-gated handlers exist (`api/reading_speed.py`, `api/estimate.py`), but the wiring was only flipped on for the e2e profile via `compose/app.e2e.yml:53` — never for the default profile. Under the default profile today: RS runs with `AUTH_TYPE=none` (archetype synthetic-admin mode); JWT signature / scope / issuer / audience are NOT validated; every bearer the BFF presents is silently accepted regardless of contents. J3 / J4 / J6 journeys would functionally pass but the **scope-enforcement surface PRD §8 + Story 5.2 §5 attest to is not exercised** — the prod-shaped smoke is auth-degraded vs. the e2e profile. This silently weakens the security review's claim that "scope enforcement is enforced at the RS" — true in the e2e profile, false in the default profile as actually shipped. **Real fix:** create `compose/app.default.yml` mirroring `compose/app.e2e.yml`'s `AUTH_TYPE: oidc_bearer` override (without the test-reset env vars) AND register it in `docker-compose.yml`'s `include:` block under a conditional, OR set `AUTH_TYPE=oidc_bearer` as the uncommented default in `services/resource-server/.env.example` (operator override to `none` for synthetic-admin dev work). **Belongs to:** Story 5.3 + a fast-follow security-correctness fix; recommend the latter as a separate small story since it touches the security posture. **Severity:** medium-to-high (the canonical prod-shaped bring-up does not actually enforce the scope rules the security review documents).

- **D139 — `/api/me` 401 envelope `message` wording is `"Session expired or not present"`** — the security review document (Story 5.2) cites the envelope by `errorCode` (`session_expired`) but does not pin the `message` text. The smoke doc's first draft (Story 5.4 Task 2.5 template) had a different placeholder (`"Your session has expired or you are not signed in."`); reconciled to actual at smoke time. Not a defect; logged for completeness so a future doc-quality pass can decide whether to standardize the `message` text across the BFF's 401 surfaces (`/api/me`, `/api/books`, etc.) or pin it byte-for-byte in `docs/security-review.md` §1. **Real fix:** none required today; if a standardization pass is desired, audit `bff/core/error_handlers.py` + `bff/core/errors.py:ErrorCode.SESSION_EXPIRED` for the canonical message string. **Belongs to:** documentation-consistency cleanup. **Severity:** nit.
