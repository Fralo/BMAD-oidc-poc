...
test.describe('J5: logout and re-protection', () => {
  ...
  test('clicking Log out terminates session and re-protects routes', async ({ page }) => {
    ...
    // Product name remains; identity affordances disappear. The top-chrome
    // brand span renders the literal "Reading Time Estimator"; the login
    // page's headline reads "Sign in to Reading Time Estimator". Lock to
    // the exact match so the assertion targets the brand only.
