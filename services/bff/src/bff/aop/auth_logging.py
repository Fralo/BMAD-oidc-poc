...
is the *only* code path in the BFF that emits decision events; the archetype's
JSON formatter inflates each entry to a full structured line that also carries
``level``, ``logger``, ``traceId``, ``spanId``.

The mirror module lives at
``services/resource-server/src/resource_server/aop/auth_logging.py`` — the
RS surface is byte-identical save for the module-name string the wire
...
# `_safe_session_id_log` (api/auth.py:83-87) uses 8 chars + "..." for any value
# longer than 0. The 12-char threshold here preserves intentionally-short test
# fixture subs verbatim while still ellipsizing realistic Keycloak UUIDs.
_SUB_TRUNCATE_AT = 12
_SUB_PREFIX_LEN = 8
_SUB_ELLIPSIS = "..."
...
def emit_auth_decision(
    *,
    ...
    reason:
        Short classifier (``"session_created"``, ``"signature_invalid"``).
