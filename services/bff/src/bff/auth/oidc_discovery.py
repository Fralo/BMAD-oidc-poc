...
lifespan hook in `bff.main` calls `fetch_discovery(...)` and stashes the
result on `app.state.oidc_discovery`; route handlers read it via the
`get_oidc_discovery` FastAPI dependency.
...
class DiscoveryFetchError(Exception):
    ...
    never echoed to an authenticated user.
    """
...
def _validated(payload: dict[str, Any], *, canonical_issuer: str) -> OidcDiscovery:
    ...
    # discovery doc could swap the issuer + endpoint URLs and the BFF would
    # trust attacker-supplied token / JWKS endpoints.
    ...
    # pathological `"token_endpoint": "/foo"` or `"javascript:..."` payloads
    # at startup rather than late at first httpx call with an opaque
