...
# `bff/aop/auth_logging.py`). The structured emit attaches them to the
# `LogRecord` via stdlib's `extra={...}`; `_lift_auth_decision_fields` runs in
# the foreign_pre_chain (BEFORE structlog's `remove_processors_meta` strips
# `_record`) and copies the fields onto the top-level `event_dict`, so the
# renderer can read them after `_record` is gone. The sentinel attribute
