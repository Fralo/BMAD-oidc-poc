...
# Architecture spec (A3) pins 5 min for the auth-state TTL. Short window
# narrows the OAuth state/nonce replay surface.
...
class SessionService:
    ...
    async def create_auth_state(
        ...
        *,
        ...
        """Persist a fresh `auth_states` row carrying `state`, `nonce`, and the
        ...
        The vestigial `code_verifier` column is left at its model default ("")
        — see Pattern Amendments in architecture.md.
        """
    ...
    async def consume_auth_state(
        ...
        *,
        ...
        # caller's later checks (cookie HMAC, token exchange) fail and return.
        await db.delete(row)
        await db.commit()
        return row
