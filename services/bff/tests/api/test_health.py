...
Story 7.2 reframed the OIDC discovery HTTP probe into a presence check
against the cached `OidcDiscovery` on `app.state` (the lifespan startup
hook already fail-fast'd on any fetch error). The exhaustive HTTP-shape
test matrix moved into `tests/auth/test_oidc_discovery.py`.
...
async def test_check_alembic_at_head_passes_when_current_matches_head(engine) -> None:
