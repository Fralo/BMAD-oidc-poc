...
BMAD_books Resource Server (reading speed, estimate)
...
```bash
uv run uvicorn resource_server.main:app --reload
```
...
docker run -p 8000:8000 resource-server
```

With custom configuration:

```bash
docker run --env-file .env -p 8000:8000 resource-server
```
...
```bash
uv run pytest
uv run pytest --cov=resource_server --cov-report=term-missing
```
...
from resource_server.models.dto import CamelCaseModel
...
from resource_server.models.entities.widget import Widget
...
```python
from resource_server.services.v1 import widget as v1_widget
...
from resource_server.core.constants import WIDGETS
from resource_server.factories.widget import (
    entity_to_get_response,
    entity_to_post_response,
    post_dto_to_entity,
)
from resource_server.models.dto.v1.widget import (
    GetWidgetsResponse,
    PostWidgetsRequest,
    PostWidgetsResponse,
)
from resource_server.services.factory import WidgetServiceV1
from resource_server.services.v1.widget_service import (
...
```python
from resource_server.api.v1.widget_routes import (
