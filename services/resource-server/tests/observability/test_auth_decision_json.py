...
def _event_dict_from_record(record: logging.LogRecord) -> MutableMapping[str, Any]:
    ...
    Invokes `_lift_auth_decision_fields` so the event_dict reflects the
    post-foreign_pre_chain state used by the production renderer
    (review-fix P1).
    """
...
def test_plain_renderer_appends_decision_fields(
    ...
    assert "reason=jwt_valid" in line
...
def test_production_pipeline_omits_decision_fields_for_non_helper_records() -> None:
    """End-to-end: unrelated `extra={"decision": ...}` records must NOT render
    decision/sub/reason (review-fix P5)."""
    buf = io.StringIO()
    handler = logging.StreamHandler(buf)
    handler.setFormatter(_build_production_formatter(_json_renderer))
